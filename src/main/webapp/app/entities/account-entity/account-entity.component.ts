...
export class AccountEntityComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.accountEntityService
      ...
      .subscribe(
        (res: HttpResponse<IAccountEntity[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInAccountEntities(): void {
    this.eventSubscriber = this.eventManager.subscribe('accountEntityListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IAccountEntity[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/account-entity'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
