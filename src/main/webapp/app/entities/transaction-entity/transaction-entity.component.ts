...
export class TransactionEntityComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.transactionEntityService
      ...
      .subscribe(
        (res: HttpResponse<ITransactionEntity[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInTransactionEntities(): void {
    this.eventSubscriber = this.eventManager.subscribe('transactionEntityListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ITransactionEntity[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/transaction-entity'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
